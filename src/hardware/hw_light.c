...
void HWR_WallLighting(FOutVector *wlVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		FOutVector *tmp = HWR_AllocVertexBuffer(4);
		memcpy(tmp, wlVerts, sizeof(FOutVector) * 4);
		HWD.pfnDrawPolygon (&Surf, tmp, 4, LIGHTMAPFLAGS);
...
void HWR_PlaneLighting(FOutVector *clVerts, int nrClipVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		FOutVector *tmp = HWR_AllocVertexBuffer(4);
		memcpy(tmp, clVerts, sizeof(FOutVector) * 4);
		HWD.pfnDrawPolygon (&Surf, tmp, nrClipVerts, LIGHTMAPFLAGS);
